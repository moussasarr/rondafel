...
if(!class_exists('Windflaw_Front')){
	...
	$windflaw_image_sizes = array(
		'phone' => array('default' => array(480, 999999), 'retina' => array(960, 999999)),
		'tablet' => array('default' => array(1024, 999999), 'retina' => array(1440, 999999)),
		'computer' => array('default' => array(1440, 999999), 'retina' => array(2880, 999999))
	);
	define('WINDFLAW_DEVICE_TYPE', ($windflaw_mobile_detect->isMobile() ? ($windflaw_mobile_detect->isTablet() ? 'tablet' : 'phone') : 'computer'));
	define('WINDFLAW_BG_IMAGE_SIZE', $windflaw_image_sizes[WINDFLAW_DEVICE_TYPE]['default']);
	define('WINDFLAW_RETINA_BG_IMAGE_SIZE', $windflaw_image_sizes[WINDFLAW_DEVICE_TYPE]['retina']);
	...
	class Windflaw_Front{
		...
		public function front_scripts_styles(){
			...
			wp_register_script('windflaw-main', WINDFLAW_THEME_JS_URI . 'front/windflaw-main.js', array('jquery'), '1.0.6', true);
			wp_localize_script('windflaw-main', 'screenReaderText', array(
				'expand' => __('expand child menu', 'windflaw-lite'),
				'collapse' => __('collapse child menu', 'windflaw-lite')
			));
			...
			wp_enqueue_style('windflaw-front', get_stylesheet_uri(), array(), '1.0.6');
			wp_enqueue_style('awsomefont', WINDFLAW_THEME_URI . 'assets/font-awesome/css/font-awesome.min.css', array(), '4.7.0');
			is_rtl() ? wp_enqueue_style('windflaw-rtl', WINDFLAW_THEME_URI . 'rtl.css', array(), '1.0.6') : '';
...
?>
			<header id="masthead" class="site-header header<?php echo has_nav_menu('primary') ? '' : ' no-menu'; ?>" role="banner" <?php if(get_header_image() != '') { echo 'style="background-image: url(' . esc_url(get_header_image()) . ');"'; } ?>>
				<div class="container">
					...
					<button id="menu-toggle" class="menu-toggle"><?php _e('Menu', 'windflaw-lite'); ?></button>
					<?php
						wp_nav_menu(array(
...
?>
			<footer class="footer <?php echo esc_attr(get_option('windflaw_theme_colors_scheme', 'light')); ?>-color-scheme" id="site-footer">
				...
				<?php endif; ?>
				<div class="site-info">&copy; <?php echo esc_html(get_bloginfo('name')) . ' ' . date('Y'); ?>. <?php printf(__('Windflaw designed by %sLoft.Ocean.%s', 'windflaw-lite'), '<a href="' . esc_url('http://www.loftocean.com'). '" alt="' . esc_attr__('WordPress Themes by Loft.Ocean', 'windflaw-lite') . '"  title="' . esc_attr__('WordPress Themes by Loft.Ocean', 'windflaw-lite') . '" rel="designer">', '</a>'); ?></div>
			</footer>
			<?php endif; ?>
		...
		*/
		public function archive_page_header(){
			if(is_author()){
				...
				if(is_search()){
					echo '<h3 class="page-title">' . __('Search Result', 'windflaw-lite') . '</h3>';
					...
					$title = __('Yearly Archive', 'windflaw-lite');
					if(!empty($vars['monthnum'])){
						$title = !empty($vars['day']) ? __('Daily Archive', 'windflaw-lite') : __('Monthly Archive', 'windflaw-lite'); 
					}
					echo '<h3 class="page-title">' . $title . '</h3>';
					echo '<p>' . sprintf(_n('%s Entry in: ', '%s Entries in: ', $founded, 'windflaw-lite'), $founded) . '<strong>';
					echo !empty($vars['year'])	 ? (' ' . $vars['year']) : '';
					echo !empty($vars['monthnum']) ? (' ' . $month[$vars['monthnum'] - 1]) : '';
					echo !empty($vars['day'])	  ? (' ' . $vars['day']) : '';
					echo '</strong></p>';
				}
				else{
					$title = is_category() ? __('Category', 'windflaw-lite')
						: (is_tag() ? __('Tag Archive', 'windflaw-lite') : __('Archive', 'windflaw-lite'));
		...
		*/
		public function get_excerpt($excerpt){
			$link = is_search() ? '' : wp_link_pages(array(
				...
				'before'      => '<div class="page-links"><span class="page-links-title">' . __('Pages:', 'windflaw-lite') . '</span>',
				'after'       => '</div>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
				'echo' => 0
		...
		*/
		...
		static function get_comment_meta($text = false){
			...
			if(($number > 0) || comments_open()){
				...
				$commentText = ($number < 1) ? __('no comment', 'windflaw-lite') : sprintf(_n('%d comment', '%d comments', $number, 'windflaw-lite'), $number);
